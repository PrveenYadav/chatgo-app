...
export async function getConversationsForSidebar (req, res) {
    try {
        ...
        const conversations = Message.aggregate([
          ...
          // 2. Collapse them into one row per chat partner, not including our latest message time.
          {
            $group: {
        ...
        console.error("Error in getting conversations for sidebar", error.message);
        res.status(500).json({message: "Internel Server Error"});
    }
}
    ...
    try {
        ...
        if (req.file) {
            if (!hasImageKitConfig()) {
                return res.staus(500).json({message: "ImageKit is not configured"}); 
            }
